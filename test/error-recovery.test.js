...
Subtotal: $10.00
    ...
    test('should handle extraction errors gracefully', () => {
      ...
      const originalExtractor = parser.extractOrderNumber;
      parser.extractOrderNumber = jest.fn(() => {
      ...
      parser.extractOrderNumber = originalExtractor;
    });
  });
  ...
  describe('Integration with parseInvoice', () => {
    test('should attempt recovery on PDF parsing failure', async () => {
      ...
      const result = await parser.parseInvoice('/fake/path/test.pdf', { silent: true });
