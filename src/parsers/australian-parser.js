/**
 * Australian Invoice Parser
 *
 * Specialized parser for Australian (AU) Amazon invoices
 * Handles Australian English patterns and AUD currency ($)
 */

const BaseParser = require('./base-parser');

class AustralianParser extends BaseParser {
  constructor() {
    super('Australian English', 'AU');
  }

  /**
   * Main extraction method for Australian invoices
   * @param {string} text - Preprocessed Australian invoice text
  ...
  extract(text) {
    ...
    if (error) {
      console.warn('Australian invoice validation warning:', error.details[0].message);
    }
  ...
  /**
   * Extract order number from Australian invoices
  ...
  extractOrderNumber(text) {
    ...
    // Australian order number patterns (similar to English)
    const orderPatterns = [
  ...
  /**
   * Extract order date from Australian invoices
   * @param {string} text - Preprocessed text
  ...
  /**
   * Extract items from Australian invoices
   * @param {string} text - Preprocessed text
  ...
  /**
   * Extract subtotal from Australian invoices
   * @param {string} text - Preprocessed text
  ...
  /**
   * Extract shipping cost from Australian invoices
   * @param {string} text - Preprocessed text
  ...
  /**
   * Extract tax from Australian invoices
   * @param {string} text - Preprocessed text
  ...
  extractTax(text) {
    ...
    // Australian tax patterns (GST - Goods and Services Tax)
    const taxPatterns = [
      /GST[:\s]*([$]\s*\d{1,3}(?:,\d{3})*\.\d{2})/i,
      /Tax[:\s]*([$]\s*\d{1,3}(?:,\d{3})*\.\d{2})/i,
      /GST\s+Included[:\s]*([$]\s*\d{1,3}(?:,\d{3})*\.\d{2})/i,
    ];
  ...
  /**
   * Extract total from Australian invoices
   * @param {string} text - Preprocessed text
  ...
  isTotalLine(text) {
    const totalKeywords = [
      ...
      'gst', 'tax', 'grand total', 'order total', 'amount due'
    ];
  ...
  calculateSubtotalFromItems(items) {
    ...
    for (const item of items) {
      if (item.price) {
        // Extract numeric value from price string (handle AUD format)
...
module.exports = AustralianParser;