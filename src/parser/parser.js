...
class AmazonInvoiceParser {
  ...
  /**
   * Check if the extracted text appears to be from a results/report PDF rather than an invoice
   * @param {string} text - Extracted text from PDF
   * @returns {boolean} - True if this appears to be a results PDF
   */
  isResultsPdf(text) {
    if (!text || typeof text !== 'string') return false;

    const resultsPdfIndicators = [
      /invoice processing report/i,
      /processing results/i,
      /export date/i,
      /summary.*successful.*failed/i,
      /validation status/i,
      /parsing quality/i,
      /generated by/i,
      /confidential.*internal use/i,
      /invoice-results-job_/i
    ];

    // Check for multiple indicators to avoid false positives
    let indicatorCount = 0;
    for (const pattern of resultsPdfIndicators) {
      if (pattern.test(text)) {
        indicatorCount++;
        if (indicatorCount >= 2) {
          return true; // High confidence this is a results PDF
        }
      }
    }
  ...
  async parseInvoice(pdfPath, options = {}) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Check if this appears to be a results/report PDF rather than an invoice
        if (this.isResultsPdf(rawInvoiceText)) {
          throw new Error('This appears to be a processed results PDF, not an invoice PDF. Please upload the original invoice PDF for parsing.');
        }
        ...
        if (invoice) {
          ...
          // Generate parsing quality checklist
          invoice.parsingQuality = this.validation.generateParsingQualityChecklist(invoice, invoice.extractionMetadata);
        }
        ...
        if (categorizedError.recoverable && !categorizedError.type.includes('file_access')) {
          ...
          try {
            ...
            if (partialData) {
              ...
              // Generate parsing quality checklist for partial data
              partialData.parsingQuality = this.validation.generateParsingQualityChecklist(partialData, partialData.extractionMetadata);
...
Zahlungsmethode: Kreditkarte ****5678
  ...
  // Backward compatibility methods - delegate to extraction module
  extractOrderNumber(text) {
    return this.extraction.extractOrderNumber(text);
  }

  extractOrderDate(text) {
    return this.extraction.extractOrderDate(text);
  }

  extractItems(text) {
    return this.extraction.extractItems(text);
  }

  extractSubtotal(text) {
    return this.extraction.extractSubtotal(text);
  }

  extractShipping(text) {
    return this.extraction.extractShipping(text);
  }

  extractTax(text) {
    return this.extraction.extractTax(text);
  }

  extractTotal(text) {
    return this.extraction.extractTotal(text);
  }

  // Backward compatibility methods for reporting and validation
  extractNumericValue(currencyString) {
    return this.validation.extractNumericValue(currencyString);
  }

  extractCurrencySymbol(amountString) {
    return this.validation.extractCurrencySymbol(amountString);
  }

  isValidCurrencyFormat(format) {
    return this.validation.isValidCurrencyFormat(format);
  }

  formatCurrency(amount, currency = 'USD') {
    return this.validation.formatCurrency(amount, currency);
  }

  generateValidationSummary(validation) {
    return this.validation.generateValidationSummary(validation);
  }

  calculateTotalSpent(invoices) {
    return this.validation.calculateTotalSpent(invoices);
  }

  generateReport(invoices) {
    return this.validation.generateReport(invoices);
  }

  validateReportOutput(report) {
    return this.validation.validateReportOutput(report);
  }
}
